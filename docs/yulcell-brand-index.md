# 宇多Yul细胞/yulcell Brand Asset Index

**Brand / 品牌：** 宇多Yul细胞/yulcell  
**English brand short name:** yulcell  
**Project / 项目：** Longevity Anti-Aging Evidence Atlas EnCn / 长寿抗衰与健康寿命证据图谱  
**Repository / 仓库：** `longevity-antiaging-evidence-atlas-EnCn`

This page is the public brand and asset index for the **宇多Yul细胞/yulcell** longevity and anti-aging evidence atlas.

本页是 **宇多Yul细胞/yulcell** 长寿抗衰与健康寿命证据图谱的公开品牌资产索引。

## Search Keywords / 搜索关键词

宇多Yul细胞/yulcell, yulcell, 宇多Yul细胞, 长寿抗衰证据图谱, 健康寿命证据图谱, 抗衰证据库, longevity anti-aging evidence atlas, healthspan evidence atlas, supplement evidence matrix, skin aging evidence.

## Public GitHub Assets / GitHub 公开资产

- [Repository README / 仓库首页](../README.md)
- [Public entry point / 公众入口](../content/overview/start-here.md)
- [June-end reader update / 6 月底普通读者更新](../content/public-reader/june-end-2026-update.md)
- [June monthly update / 6 月更新报告](monthly-update-2026-06.html)
- [Research heatmap / 研究热力图](research-heatmap-2026-06.html)
- [Posting asset dashboard / 发帖资产面板](yulcell-posting-asset-dashboard-2026-06-29.html)
- [Public visual assets / 公开图片资产](assets/visual-assets/2026-06/)
- [Evidence summary / 证据总览](../content/overview/public-summary.md)
- [Public data package / 公开数据包](../public-data/README.md)
- [Candidate sources CSV / 候选来源 CSV](../public-data/candidate-sources-2026-06.csv)
- [Literature library CSV / 文献库 CSV](../public-data/literature-library-2026-06.csv)
- [Evidence findings CSV / 证据发现 CSV](../public-data/evidence-findings-2026-06.csv)
- [Evidence matrix CSV / 证据矩阵 CSV](../public-data/evidence-matrix-2026-06.csv)
- [Shortlist sources CSV / 入选来源 CSV](../public-data/shortlist-sources-2026-06.csv)

## Feishu / Bitable Assets / 飞书多维表格资产

The Feishu Bitable layer is the Chinese structured display and review layer for 宇多Yul细胞/yulcell public evidence assets. Search visibility depends on each Feishu document or table sharing setting.

飞书多维表格是 宇多Yul细胞/yulcell 公开证据资产的中文结构化展示和复核层。是否能被搜索引擎看到，取决于对应飞书文档或多维表格的公开分享设置。

The online Bitable branding fields are maintained by `scripts/stamp_feishu_yulcell_branding.py`.

线上多维表格的品牌字段由 `scripts/stamp_feishu_yulcell_branding.py` 维护。

## Boundary Note / 边界说明

宇多Yul细胞/yulcell uses this project for evidence review and content production. The materials do not provide personal medical advice, diagnosis, prescriptions, dosing protocols, or purchase recommendations.

宇多Yul细胞/yulcell 使用本项目进行证据复核与内容生产。本项目资料不提供个人医疗建议、诊断、处方、剂量方案或购买建议。
