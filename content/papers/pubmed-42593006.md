# Association of sleep duration and hypertension in older adults: a cross-sectional study.

> 草稿状态：自动整理，尚未完成全文复核，不构成医疗建议。
> Draft status: automatically prepared; not fully reviewed; not medical advice.

## Metadata / 元数据

- Candidate ID: `pubmed-42593006`
- Source: `PubMed`
- PMID: `42593006`
- PMCID: ``
- DOI: `10.1080/08037051.2026.2717792`
- Year: 2026
- Journal/Registry: Blood pressure
- Topic: 血压与健康寿命 / Blood Pressure and Healthspan
- Evidence source depth: `abstract_only`

## Study Design / 研究设计

- Draft study type: `metadata_only_needs_classification`
- Draft species/population: `human` / 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft intervention or exposure: blood pressure
- Draft comparator: 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft primary endpoint: H4
- Draft sample size: 摘要级/题录级待复核 / abstract-or-metadata-level pending review

## Main Results / 主要结果

EN: Among 15,650 participants (mean age 87.16 years, SD 11.36; 57.0% female), 7,345 (46.9%) met criteria for hypertension. In the fully adjusted model, short sleep duration (<6 h) was associated with a 1.30-fold higher odds of hypertension compared with sleeping 6-9 h (adjusted odds ratio [OR] 1.30; 95% confidence interval [CI] 1.17, 1.44). Long sleep duration (≥9 h) showed no statistically significant association after full adjustment (OR 1.04; 95% CI 0.97, 1.12). The relationship followed a non-linear J-shaped curve (p for non-linearity < 0.05). Interaction analyses revealed that the effect of short sleep duration on hypertension was more pronounced among adults with no limitations in activities of daily living (p for interaction = 0.009).

ZH draft: 中文草稿：该研究属于「血压与健康寿命」主题；摘要/题录结果显示：Among 15,650 participants (mean age 87.16 years, SD 11.36; 57.0% female), 7,345 (46.9%) met criteria for hypertension. In the fully adjusted model, short sleep duration (<6 h) was associated with a 1.30-fold higher odds of hypertension compared with sleeping 6-9 h (adjusted odds ratio [OR] 1.30; 95% confidence interval [CI] 1.17, 1.44). Long sleep duration (≥9 h) showed no statistically significant association after full adjustment (OR 1.04; 95% CI 0.97, 1.12). The relationship followed a non...

## Conclusion Boundary / 结论边界

EN: Short sleep duration (<6 h) is independently associated with hypertension in Chinese older adults. These findings suggest sleep assessment may help identify older adults at hypertension risk, and support future trials on sleep hygiene for hypertension prevention.

ZH draft: 中文草稿：英文摘要结论为：Short sleep duration (<6 h) is independently associated with hypertension in Chinese older adults. These findings suggest sleep assessment may help identify older adults at hypertension risk, and support future trials on sleep hygiene for hypertension prevention.

## Supported Claim / 支持的结论

- ZH: 可支持：将「血压与健康寿命」作为证据图谱中的候选主题，并按 E 级草判证据继续复核。
- EN: Supports treating Blood Pressure and Healthspan as a candidate evidence topic with draft level E, pending full review.

## Unsupported Claim / 不支持的结论

- ZH: 不支持：不能把替代指标或 biomarker 改善直接解释为临床逆龄。
- EN: Does not support interpreting surrogate or biomarker change as clinical rejuvenation.

## Overinterpretation Risk / 过度解读风险

- ZH: 过度解读风险：自动抽取结果不能替代全文复核，不能直接转化为个人医疗建议。
- EN: Overinterpretation risk: automated extraction does not replace full-text review and cannot be converted into personal medical advice.

## v0.4/v0.5 Scoring / 评分

- Draft evidence level: `E`
- Final evidence level: `E`
- Endpoint class: `H4`
- Quality confidence score: `26`
- Influence score: `0`
- Confidence cap rule: `max_level_A`
- Risk-of-bias tool: `domain_screen`
- Medical supervision needed: `false`
- Authority signal: DOI; PMID; peer-reviewed journal metadata

## Review Status / 复核状态

Public draft. Needs human full-text review before formal recommendation or clinical interpretation.

公开草稿。形成正式推荐或临床解释前，必须人工阅读全文复核。
