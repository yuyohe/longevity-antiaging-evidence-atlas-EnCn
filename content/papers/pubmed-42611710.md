# Exercise to prevent falls in older adults with physical frailty: A systematic review and meta-analysis.

> 草稿状态：自动整理，尚未完成全文复核，不构成医疗建议。
> Draft status: automatically prepared; not fully reviewed; not medical advice.

## Metadata / 元数据

- Candidate ID: `pubmed-42611710`
- Source: `PubMed`
- PMID: `42611710`
- PMCID: `PMC13507850`
- DOI: `10.1016/j.tjfa.2026.100187`
- Year: 2026
- Journal/Registry: The Journal of frailty & aging
- Topic: 抗阻训练、肌肉与衰弱 / Resistance Training, Muscle, and Frailty
- Evidence source depth: `abstract_plus_open_pmc_available`

## Study Design / 研究设计

- Draft study type: `systematic_review_or_meta_analysis`
- Draft species/population: `human` / 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft intervention or exposure: resistance training / muscle
- Draft comparator: 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft primary endpoint: H2
- Draft sample size: 摘要级/题录级待复核 / abstract-or-metadata-level pending review

## Main Results / 主要结果

EN: Fourteen studies (n = 2719; mean age range 73-86 years) were included, most of which used the Fried frailty phenotype (n = 12) and were conducted in Europe (n = 6). Most interventions included multicomponent (n = 10) and resistance (n = 4) training. Exercise improved gait speed (MD 0.09 m/s, 0.02/0.15; I2=94%; n = 5), TUG (MD -3.73 s, -5.92/-1.55; I2=95%; n = 6), and 5-times sit-to-stand (MD -2.66 s, -4.19/-1.14; I2=77%; n = 4) over short-term period (≤12 weeks), with low-to-very low certainty evidence. A trend toward improved balance was also noted (SMD 2.80, -0.17/5.76; p = 0.06; I²=99%; n = 5), with very low certainty evidence. Limited data was observed for fear of falling (n = 1) and falls incidence (n = 2).

ZH draft: 中文草稿：该研究属于「抗阻训练、肌肉与衰弱」主题；摘要/题录结果显示：Fourteen studies (n = 2719; mean age range 73-86 years) were included, most of which used the Fried frailty phenotype (n = 12) and were conducted in Europe (n = 6). Most interventions included multicomponent (n = 10) and resistance (n = 4) training. Exercise improved gait speed (MD 0.09 m/s, 0.02/0.15; I2=94%; n = 5), TUG (MD -3.73 s, -5.92/-1.55; I2=95%; n = 6), and 5-times sit-to-stand (MD -2.66 s, -4.19/-1.14; I2=77%; n = 4) over short-term period (≤12 weeks), with low-to-very low certaint...

## Conclusion Boundary / 结论边界

EN: Exercise training enhances physical function in older adults with physical frailty. However, its impact on fear of falling and fall incidence remains uncertain.

ZH draft: 中文草稿：英文摘要结论为：Exercise training enhances physical function in older adults with physical frailty. However, its impact on fear of falling and fall incidence remains uncertain.

## Supported Claim / 支持的结论

- ZH: 可支持：将「抗阻训练、肌肉与衰弱」作为证据图谱中的候选主题，并按 A 级草判证据继续复核。
- EN: Supports treating Resistance Training, Muscle, and Frailty as a candidate evidence topic with draft level A, pending full review.

## Unsupported Claim / 不支持的结论

- ZH: 不支持：不能据单篇摘要给出剂量、处方或个人医疗建议。
- EN: Does not support dosing, prescriptions, or individual medical advice from one abstract.

## Overinterpretation Risk / 过度解读风险

- ZH: 过度解读风险：自动抽取结果不能替代全文复核，不能直接转化为个人医疗建议。
- EN: Overinterpretation risk: automated extraction does not replace full-text review and cannot be converted into personal medical advice.

## v0.4/v0.5 Scoring / 评分

- Draft evidence level: `A`
- Final evidence level: `A`
- Endpoint class: `H2`
- Quality confidence score: `85`
- Influence score: `0`
- Confidence cap rule: `max_level_A`
- Risk-of-bias tool: `AMSTAR 2`
- Medical supervision needed: `false`
- Authority signal: DOI; PMID; PMCID/open-access signal; peer-reviewed journal metadata; abstract_plus_open_pmc_available

## Review Status / 复核状态

Public draft. Needs human full-text review before formal recommendation or clinical interpretation.

公开草稿。形成正式推荐或临床解释前，必须人工阅读全文复核。
