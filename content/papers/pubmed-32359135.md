# Association between Mediterranean diet adherence and colorectal cancer: a dose-response meta-analysis.

> 草稿状态：自动整理，尚未完成全文复核，不构成医疗建议。
> Draft status: automatically prepared; not fully reviewed; not medical advice.

## Metadata / 元数据

- Candidate ID: `pubmed-32359135`
- Source: `PubMed`
- PMID: `32359135`
- PMCID: ``
- DOI: `10.1093/ajcn/nqaa083`
- Year: 2020
- Journal/Registry: The American journal of clinical nutrition
- Venue tier: `A_field_leading`
- Venue score: `7`
- Venue note: Leading aging, nutrition, metabolism, geriatrics, dermatology, or translational specialty venue.
- Topic: 饮食模式与死亡风险 / Dietary Patterns and Mortality
- Evidence source depth: `abstract_only`

## Study Design / 研究设计

- Draft study type: `systematic_review_or_meta_analysis`
- Draft species/population: `human` / 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft intervention or exposure: dietary pattern
- Draft comparator: 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft primary endpoint: H1
- Draft sample size: 摘要级/题录级待复核 / abstract-or-metadata-level pending review

## Main Results / 主要结果

EN: This meta-analysis included 13 prospective cohort studies, of which 9 reported CRC incidence and 5 reported CRC mortality. The summary RR of CRC incidence was 0.90 (95% CI: 0.84, 0.96) for highest compared with lowest MD adherence and 0.96 (95% CI: 0.94, 0.99) per 2-score increase in MD adherence. The summary RRs for highest compared with lowest MD adherence were 0.82 for rectal cancer (95% CI: 0.71, 0.95), 0.94 for proximal colon cancer (95% CI: 0.87, 1.02), and 0.91 for distal colon cancer (95% CI: 0.79, 1.04). Neither the summary HR of overall mortality for highest compared with lowest pre- and postdiagnosis MD adherence, nor the summary HR of CRC-specific mortality for highest compared with lowest prediagnosis MD adherence achieved a value with statistical significance.

ZH draft: 中文草稿：该研究属于「饮食模式与死亡风险」主题；摘要/题录结果显示：This meta-analysis included 13 prospective cohort studies, of which 9 reported CRC incidence and 5 reported CRC mortality. The summary RR of CRC incidence was 0.90 (95% CI: 0.84, 0.96) for highest compared with lowest MD adherence and 0.96 (95% CI: 0.94, 0.99) per 2-score increase in MD adherence. The summary RRs for highest compared with lowest MD adherence were 0.82 for rectal cancer (95% CI: 0.71, 0.95), 0.94 for proximal colon cancer (95% CI: 0.87, 1.02), and 0.91 for distal colon cancer...

## Conclusion Boundary / 结论边界

EN: Our meta-analysis supports the inverse association of MD adherence with CRC incidence, but not with overall mortality or CRC-specific mortality among those diagnosed with CRC.

ZH draft: 中文草稿：英文摘要结论为：Our meta-analysis supports the inverse association of MD adherence with CRC incidence, but not with overall mortality or CRC-specific mortality among those diagnosed with CRC.

## Supported Claim / 支持的结论

- ZH: 可支持：将「饮食模式与死亡风险」作为证据图谱中的候选主题，并按 A 级草判证据继续复核。
- EN: Supports treating Dietary Patterns and Mortality as a candidate evidence topic with draft level A, pending full review.

## Unsupported Claim / 不支持的结论

- ZH: 不支持：不能据单篇摘要给出剂量、处方或个人医疗建议。
- EN: Does not support dosing, prescriptions, or individual medical advice from one abstract.

## Overinterpretation Risk / 过度解读风险

- ZH: 过度解读风险：自动抽取结果不能替代全文复核，不能直接转化为个人医疗建议。
- EN: Overinterpretation risk: automated extraction does not replace full-text review and cannot be converted into personal medical advice.

## v0.5 Scoring / 评分

- Draft evidence level: `A`
- Final evidence level: `A`
- Endpoint class: `H1`
- Quality confidence score: `88`
- Influence score: `0`
- Venue tier: `A_field_leading`
- Venue score: `7`
- Confidence cap rule: `max_level_A`
- Risk-of-bias tool: `AMSTAR 2`
- Medical supervision needed: `false`
- Authority signal: DOI; PMID; review-level publication type; peer-reviewed journal metadata

## Review Status / 复核状态

Public draft. Needs human full-text review before formal recommendation or clinical interpretation.

公开草稿。形成正式推荐或临床解释前，必须人工阅读全文复核。
