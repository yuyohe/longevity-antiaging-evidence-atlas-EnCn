# Safety and efficacy of umbilical cord tissue-derived mesenchymal stem cells in the treatment of patients with aging frailty: a phase I/II randomized, double-blind, placebo-controlled study.

> 草稿状态：自动整理，尚未完成全文复核，不构成医疗建议。
> Draft status: automatically prepared; not fully reviewed; not medical advice.

## Metadata / 元数据

- Candidate ID: `pubmed-38679727`
- Source: `PubMed`
- PMID: `38679727`
- PMCID: `PMC5009659`
- DOI: `10.1186/s13287-016-0386-0`
- Year: 2024
- Journal/Registry: Stem cell research & therapy
- Venue tier: `C_indexed_or_identified`
- Venue score: `1`
- Venue note: Indexed or DOI/PMID-identifiable venue, but not in the curated high-weight venue list.
- Topic: 微生物组与炎症性衰老 / Microbiome and Inflammaging
- Evidence source depth: `abstract_plus_open_pmc_available`

## Study Design / 研究设计

- Draft study type: `human_randomized_or_clinical_trial`
- Draft species/population: `human` / 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft intervention or exposure: microbiome / inflammaging
- Draft comparator: 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft primary endpoint: H2
- Draft sample size: 摘要级/题录级待复核 / abstract-or-metadata-level pending review

## Main Results / 主要结果

EN: In the MSCs group, significant improvements in PCS of SF-36 were observed from first post-treatment visit and sustained throughout the follow-up period, with greater changes compared to the placebo group (p = 0.042). EQ-VAS scores of MSCs group improved significantly at 2 month (p = 0.023) and continued until the end of the 6-month visit (p = 0.002) in comparison to the placebo group. The timed up and go (TUG) physical performance test revealed significant group difference and showed continual enhancements over 6 months (p < 0.05). MSC transplantation improved the function of 4-m walking test (4MWT) compared with the placebo group with a decrease of 2.05 s at 6 months of follow-up (p = 0.21). The measurement of grip strength revealed group difference with MSCs group demonstrating better performance, particularly at 6 months (p = 0.002). Inflammatory cytokines (TNF-α, IL-17) exhibited...

ZH draft: 中文草稿：该研究属于「微生物组与炎症性衰老」主题；摘要/题录结果显示：In the MSCs group, significant improvements in PCS of SF-36 were observed from first post-treatment visit and sustained throughout the follow-up period, with greater changes compared to the placebo group (p = 0.042). EQ-VAS scores of MSCs group improved significantly at 2 month (p = 0.023) and continued until the end of the 6-month visit (p = 0.002) in comparison to the placebo group. The timed up and go (TUG) physical performance test revealed significant group difference and showed continua...

## Conclusion Boundary / 结论边界

EN: Intravenous transplantation of HUC-MSCs is a safe and effective therapeutic approach on aging frailty. The positive outcomes observed in improving quality of life, physical performance, and reducing chronic inflammation, suggest that HUC-MSC therapy may be a promising potential treatment option for aging frailty.

ZH draft: 中文草稿：英文摘要结论为：Intravenous transplantation of HUC-MSCs is a safe and effective therapeutic approach on aging frailty. The positive outcomes observed in improving quality of life, physical performance, and reducing chronic inflammation, suggest that HUC-MSC therapy may be a promising potential treatment option for aging frailty.

## Supported Claim / 支持的结论

- ZH: 可支持：将「微生物组与炎症性衰老」作为证据图谱中的候选主题，并按 B 级草判证据继续复核。
- EN: Supports treating Microbiome and Inflammaging as a candidate evidence topic with draft level B, pending full review.

## Unsupported Claim / 不支持的结论

- ZH: 不支持：不能据单篇摘要给出剂量、处方或个人医疗建议。
- EN: Does not support dosing, prescriptions, or individual medical advice from one abstract.

## Overinterpretation Risk / 过度解读风险

- ZH: 过度解读风险：自动抽取结果不能替代全文复核，不能直接转化为个人医疗建议。
- EN: Overinterpretation risk: automated extraction does not replace full-text review and cannot be converted into personal medical advice.

## v0.5 Scoring / 评分

- Draft evidence level: `B`
- Final evidence level: `B`
- Endpoint class: `H2`
- Quality confidence score: `81`
- Influence score: `0`
- Venue tier: `C_indexed_or_identified`
- Venue score: `1`
- Confidence cap rule: `max_level_B`
- Risk-of-bias tool: `Cochrane RoB 2`
- Medical supervision needed: `false`
- Authority signal: DOI; PMID; PMCID/open-access signal; trial publication type; peer-reviewed journal metadata; abstract_plus_open_pmc_available

## Review Status / 复核状态

Public draft. Needs human full-text review before formal recommendation or clinical interpretation.

公开草稿。形成正式推荐或临床解释前，必须人工阅读全文复核。
