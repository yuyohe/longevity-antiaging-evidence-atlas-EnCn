# Shared and niche-specific transcriptional signatures of macrophage aging revealed by a cross-tissue meta-analysis.

> 草稿状态：自动整理，尚未完成全文复核，不构成医疗建议。
> Draft status: automatically prepared; not fully reviewed; not medical advice.

## Metadata / 元数据

- Candidate ID: `pubmed-42449369`
- Source: `PubMed`
- PMID: `42449369`
- PMCID: `PMC10639077`
- DOI: `10.1093/nar/gkad1025`
- Year: 2026
- Journal/Registry: BMC biology
- Topic: 微生物组与炎症性衰老 / Microbiome and Inflammaging
- Evidence source depth: `abstract_plus_open_pmc_available`

## Study Design / 研究设计

- Draft study type: `systematic_review_or_meta_analysis`
- Draft species/population: `human` / 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft intervention or exposure: microbiome / inflammaging
- Draft comparator: 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft primary endpoint: H4
- Draft sample size: 摘要级/题录级待复核 / abstract-or-metadata-level pending review

## Main Results / 主要结果

EN: The similarity of differentially expressed aging genes was compared across niches and pathway-level analysis uncovered conserved age-associated signatures, including upregulation of gene sets related to antigen presentation, antioxidant responses, and negative regulation of ferroptosis, alongside downregulation of gene sets related to Wnt, GTPase, and extracellular matrix organization signaling. Transcription factor activity inference identified consistent age-associated activation of AP-1 (Fos, Jun), C/EBPβ, PU.1, and Egr1 across niches. Meta-analysis defined 593 consistently age-altered genes in > 3/4 of analyzed datasets, converging on dysregulation of small GTPase signaling. Focused analysis of alveolar macrophages and microglia, made possible by the larger number of available datasets, revealed sex-specific transcriptional programs altered with age in these macrophage subtypes.

ZH draft: 中文草稿：该研究属于「微生物组与炎症性衰老」主题；摘要/题录结果显示：The similarity of differentially expressed aging genes was compared across niches and pathway-level analysis uncovered conserved age-associated signatures, including upregulation of gene sets related to antigen presentation, antioxidant responses, and negative regulation of ferroptosis, alongside downregulation of gene sets related to Wnt, GTPase, and extracellular matrix organization signaling. Transcription factor activity inference identified consistent age-associated activation of AP-1 (F...

## Conclusion Boundary / 结论边界

EN: These findings demonstrate that macrophage aging is shaped by both tissue niche and sex and provides a framework for understanding the transcriptomic signatures of macrophage aging across tissues.

ZH draft: 中文草稿：英文摘要结论为：These findings demonstrate that macrophage aging is shaped by both tissue niche and sex and provides a framework for understanding the transcriptomic signatures of macrophage aging across tissues.

## Supported Claim / 支持的结论

- ZH: 可支持：将「微生物组与炎症性衰老」作为证据图谱中的候选主题，并按 E 级草判证据继续复核。
- EN: Supports treating Microbiome and Inflammaging as a candidate evidence topic with draft level E, pending full review.

## Unsupported Claim / 不支持的结论

- ZH: 不支持：不能把替代指标或 biomarker 改善直接解释为临床逆龄。
- EN: Does not support interpreting surrogate or biomarker change as clinical rejuvenation.

## Overinterpretation Risk / 过度解读风险

- ZH: 过度解读风险：自动抽取结果不能替代全文复核，不能直接转化为个人医疗建议。
- EN: Overinterpretation risk: automated extraction does not replace full-text review and cannot be converted into personal medical advice.

## v0.4/v0.5 Scoring / 评分

- Draft evidence level: `E`
- Final evidence level: `B`
- Endpoint class: `H4`
- Quality confidence score: `74`
- Influence score: `0`
- Confidence cap rule: `max_level_B`
- Risk-of-bias tool: `AMSTAR 2`
- Medical supervision needed: `false`
- Authority signal: DOI; PMID; PMCID/open-access signal; review-level publication type; peer-reviewed journal metadata; abstract_plus_open_pmc_available

## Review Status / 复核状态

Public draft. Needs human full-text review before formal recommendation or clinical interpretation.

公开草稿。形成正式推荐或临床解释前，必须人工阅读全文复核。
