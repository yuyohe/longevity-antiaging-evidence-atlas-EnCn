# Percentage of Age-Predicted Cardiorespiratory Fitness Is Inversely Associated with Cardiovascular Disease Mortality: A Prospective Cohort Study.

> 草稿状态：自动整理，尚未完成全文复核，不构成医疗建议。
> Draft status: automatically prepared; not fully reviewed; not medical advice.

## Metadata / 元数据

- Candidate ID: `pubmed-34198307`
- Source: `PubMed`
- PMID: `34198307`
- PMCID: ``
- DOI: `10.1159/000516123`
- Year: 2021
- Journal/Registry: Cardiology
- Venue tier: `C_indexed_or_identified`
- Venue score: `1`
- Venue note: Indexed or DOI/PMID-identifiable venue, but not in the curated high-weight venue list.
- Topic: 心肺适能与死亡风险 / Cardiorespiratory Fitness and Mortality
- Evidence source depth: `abstract_only`

## Study Design / 研究设计

- Draft study type: `metadata_only_needs_classification`
- Draft species/population: `human` / 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft intervention or exposure: fitness / VO2max
- Draft comparator: 摘要级/题录级待复核 / abstract-or-metadata-level pending review
- Draft primary endpoint: H1
- Draft sample size: 摘要级/题录级待复核 / abstract-or-metadata-level pending review

## Main Results / 主要结果

EN: During a median follow-up of 28.5 years, 643 fatal CVDs were recorded. The relationship between % age-predicted CRF and CVD mortality was dose response in nature. In analysis adjusted for conventional risk factors, one standard deviation increase in % age-predicted CRF was associated with a reduced risk of CVD mortality (HR 0.61; 95% CI: 0.56-0.67), which was minimally attenuated on further adjustment for several other confounders (HR 0.71; 95% CI: 0.64-0.78). Addition of % age-predicted CRF to a CVD mortality risk prediction model containing established risk factors significantly improved risk discrimination and reclassification.

ZH draft: 中文草稿：该研究属于「心肺适能与死亡风险」主题；摘要/题录结果显示：During a median follow-up of 28.5 years, 643 fatal CVDs were recorded. The relationship between % age-predicted CRF and CVD mortality was dose response in nature. In analysis adjusted for conventional risk factors, one standard deviation increase in % age-predicted CRF was associated with a reduced risk of CVD mortality (HR 0.61; 95% CI: 0.56-0.67), which was minimally attenuated on further adjustment for several other confounders (HR 0.71; 95% CI: 0.64-0.78). Addition of % age-predicted CRF...

## Conclusion Boundary / 结论边界

EN: Percentage of age-predicted CRF is inversely and independently associated with CVD mortality in a graded fashion and significantly improves the prediction and classification of the long-term risk for CVD mortality beyond established risk factors.

ZH draft: 中文草稿：英文摘要结论为：Percentage of age-predicted CRF is inversely and independently associated with CVD mortality in a graded fashion and significantly improves the prediction and classification of the long-term risk for CVD mortality beyond established risk factors.

## Supported Claim / 支持的结论

- ZH: 可支持：将「心肺适能与死亡风险」作为证据图谱中的候选主题，并按 E 级草判证据继续复核。
- EN: Supports treating Cardiorespiratory Fitness and Mortality as a candidate evidence topic with draft level E, pending full review.

## Unsupported Claim / 不支持的结论

- ZH: 不支持：不能据单篇摘要给出剂量、处方或个人医疗建议。
- EN: Does not support dosing, prescriptions, or individual medical advice from one abstract.

## Overinterpretation Risk / 过度解读风险

- ZH: 过度解读风险：自动抽取结果不能替代全文复核，不能直接转化为个人医疗建议。
- EN: Overinterpretation risk: automated extraction does not replace full-text review and cannot be converted into personal medical advice.

## v0.5 Scoring / 评分

- Draft evidence level: `E`
- Final evidence level: `D`
- Endpoint class: `H1`
- Quality confidence score: `43`
- Influence score: `0`
- Venue tier: `C_indexed_or_identified`
- Venue score: `1`
- Confidence cap rule: `max_level_A`
- Risk-of-bias tool: `domain_screen`
- Medical supervision needed: `false`
- Authority signal: DOI; PMID; peer-reviewed journal metadata

## Review Status / 复核状态

Public draft. Needs human full-text review before formal recommendation or clinical interpretation.

公开草稿。形成正式推荐或临床解释前，必须人工阅读全文复核。
